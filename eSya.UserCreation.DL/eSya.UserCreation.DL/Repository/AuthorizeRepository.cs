...
namespace eSya.UserCreation.DL.Repository
{
    public class AuthorizeRepository:IAuthorizeRepository
    {
        ...
        public async Task<List<DO_UserMaster>> GetUnAuthenticatedUsers()
        {
            try
            {
                using (var db = new eSyaEnterprise())
                {
                    var ds = db.GtEuusms.Where(x => x.IsUserAuthenticated == false && x.ActiveStatus == true)
                        .Select(r => new DO_UserMaster
                        {
                            ...
                            UserAuthenticatedDate = (DateTime)r.UserAuthenticatedDate,
